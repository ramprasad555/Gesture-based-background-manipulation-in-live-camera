...
def pick_and_merge_objects(file_path, output_file, num_objects=5):
    ...
    selected_like_objects = dict(random.sample(sorted(like_objects.items()), min(num_objects, len(like_objects))))
    selected_dislike_objects = dict(random.sample(sorted(dislike_objects.items()), min(num_objects, len(dislike_objects))))
...
pick_and_merge_objects('output.json', 'selected_objects.json', num_objects=5)
